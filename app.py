...
def generate_roadmap():
    ...
    try:
        roadmap_data = ai_service.generate_roadmap(
            hobby=hobby, goal=goal, experience=experience
        )
        ...
        formatted_edges = [
            {
                "from": e.get("from_node") or e.get("from"),
        ...
        return jsonify(
            {
                "status": "success",
                "data": {
                    "roadmap_title": roadmap_data.get(
                        "roadmap_title", f"{hobby} Explorer"
                    ),
                    "nodes": nodes,
            ...
            formatted_nodes.append(
                {
                    ...
                    "label": sub_label,
                    "status": "current"
                    if item.get("is_immediate_prereq", True)
                    else "locked",
                }
            )

            formatted_edges.append({"from": target_id, "to": new_id})

        return jsonify({"nodes": formatted_nodes, "edges": formatted_edges}), 200
...
def generate_resources():
    ...
    try:
        result = ai_service.generate_resources(
            topic=topic, roadmap_title=roadmap_title, preferences=preferences
        )
...
def breakdown_step():
    ...
    try:
        sub_steps = ai_service.breakdown_step(
            target_node_id=target_node_id, last_finished_node_id=last_finished_node_id
        )
